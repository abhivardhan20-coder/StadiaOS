...
describe('ApiClient', () => {
  ...
  describe('getGreenOpsData', () => {
    it('returns parsed JSON on a successful request', async () => {
      (fetch as any).mockResolvedValueOnce({ ok: true, json: async () => ({ score: 85 }) });
  ...
  describe('translate', () => {
    it('returns parsed JSON on a successful request', async () => {
      (fetch as any).mockResolvedValueOnce({ ok: true, json: async () => ({ translatedText: 'Hola' }) });
    ...
    it('handles non-2xx error with message', async () => {
      (fetch as any).mockResolvedValueOnce({ ok: false, status: 400, json: async () => ({ message: 'Bad Request' }) });
      const result = await ApiClient.translate('Hello', 'es');
      expect(result).toBeDefined(); // Falls back to mock on throw
    });
    
    it('handles non-2xx error without json', async () => {
      (fetch as any).mockResolvedValueOnce({ ok: false, status: 502, json: async () => { throw new Error('invalid') } });
      const result = await ApiClient.translate('Hello', 'es');
      expect(result).toBeDefined(); // Falls back to mock on throw
    });
  });
  ...
  describe('getStadiumLive', () => {
    it('returns parsed JSON on a successful request', async () => {
      (fetch as any).mockResolvedValueOnce({ ok: true, json: async () => ({ occupancy: 100 }) });
  ...
  describe('getWayfinderRoute', () => {
    it('returns parsed JSON on a successful request', async () => {
      (fetch as any).mockResolvedValueOnce({ ok: true, json: async () => ({ destination: 'Gate A' }) });
